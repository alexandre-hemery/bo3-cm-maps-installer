...
class Application(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.workshop_id_label = tk.Label(self, text="Enter the Workshop ID of the item you wish to download:")
    ...
    def set_game_directory(self):
        game_directory = filedialog.askdirectory(title="Select the directory of the Call of Duty Black Ops III game")
        if game_directory:
    ...
    def download_workshop_item(self, workshop_id):
        try:
            ...
            messagebox.showerror("Error", "The download failed. Please try again.")
    ...
    def start_download(self):
        ...
        if game_directory is not None:
            self.download_status_var.set("Download in progress...")
            download_thread = threading.Thread(target=self.download_and_install, args=(workshop_id, game_directory), daemon=True)
            download_thread.start()
            self.after(100, self.check_download_status, download_thread)
            
    def check_download_status(self, download_thread):
    ...
    def download_and_install(self, workshop_id, game_directory):
        self.after_idle(self.download_status_var.set, "Download in progress...")
    ...
    def open_changelog(self, event):
        webbrowser.open("https://github.com/alexandre-hemery/bo3-cm-maps-installer/releases/tag/release-1.2")
    ...
    def confirm_delete(self):
        confirmation = messagebox.askyesno("Confirmation", "This action will delete all files and folders in the directory, except for the script, steamcmd.exe, and the 'game_directory.txt' file that allows the program to remember the game directory. Please use this functionality for debugging purposes only, in case steamcmd.exe is unresponsive and refuses to download the requested item despite multiple attempts. On the next run of the program, steamcmd.exe will proceed to download its essential files, as if it were its first use. Do you wish to proceed?")
        if confirmation:
            self.delete_files()
